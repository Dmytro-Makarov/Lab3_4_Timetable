...
from CSP.data import *


def run_benchmark(heuristic):
    return backtrack(heuristic, domains_start(), Schedule([], [], []))
    ...
    for day in week_schedule.keys():
        for time_slot in time_schedule.keys():
            for room in classrooms:
                buf.append(DomainElement(day, time_slot, room))
                if room.is_big:
                    buf_lecture.append(DomainElement(day, time_slot, room))
...
def backtrack(heuristic, domain, schedule):
    ...
    for d in domain[ind]:
        ...
        dom_copy.pop(ind)
        dom_copy = upd_domain(dom_copy, lessons[ind], d.day, d.time, d.room)
...
def upd_domain(domain, lesson, day, time, room):
    for key in domain:
        buf = []
        for d in domain[key]:
